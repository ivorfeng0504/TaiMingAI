...
namespace TaiMingAI.DBTools
{
    public class DBContext
    {
        ...
        private void AssignParameterValues(SqlParameter[] commandParameters, object[] parameterValues)
        {
            ...
            {
                // 如果当前数组值来自IDbDataParameter，则分配其值属性
        ...
        /// <param name="tableNames">表映射的数据表数组 用户定义的表名(可以是实际的表名.)</param>   
                ...
                if (tableNames != null && tableNames.Length > 0)
                {
                    ...
                    for (int index = 0; index < tableNames.Length; index++)
                    {
                        if (tableNames[index] == null || tableNames[index].Length == 0)
                            throw new ArgumentException("tableNames参数必须包含一个表的列表，该值被提供为null或空字符串。", "tableNames");
